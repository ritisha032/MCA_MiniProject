...
const totalMealCount = async (req, res) => {
    const allCoupons = await Coupon.find({}); // Get all purchased coupons  
    let toMake = [
...
const couponPurchase = async (req, res) => {
    const data = await Coupon.updateOne({ email: req.body.email }, { $set: { week: req.body.selected, taken: true } }, { upsert: true })
    res.send(data);
}
